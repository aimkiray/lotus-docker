...
RUN export PATH=$PATH:/usr/local/go/bin:$HOME/.cargo/bin \
    ...
    && env RUSTFLAGS='-C target-cpu=native -g' FIL_PROOFS_USE_GPU_COLUMN_BUILDER=1 FIL_PROOFS_USE_GPU_TREE_BUILDER=1 FFI_BUILD_FROM_SOURCE=1 make clean && make lotus-miner
...
ENV LOTUS_STORAGE_PATH=/lotus/lotus-miner
...
# Lotus bin & config
COPY --from=builder /lotus/lotus-miner /usr/local/bin/lotus-miner
